...
class InstalledAppFlow(Flow):
    ...
    def run_local_server(
        ...
        print(authorization_prompt_message)
        self.fetch_token(authorization_response=authorization_response)

        return self.credentials, authorization_response


    def run_local_server_1(self, host='localhost', port=8080,
            authorization_prompt_message=_DEFAULT_AUTH_PROMPT_MESSAGE,
            success_message=_DEFAULT_WEB_SUCCESS_MESSAGE,
            open_browser=True,
            **kwargs):
        print("run1")
        # success_message = "https://127.0.0.1:5000/callback"
        wsgi_app = _RedirectWSGIApp(success_message)
        local_server = wsgiref.simple_server.make_server(
            host, port, wsgi_app, handler_class=_WSGIRequestHandler)
        self.redirect_uri = 'http://{}:{}/'.format(
                host, local_server.server_port)
        auth_url, _ = self.authorization_url(**kwargs)
        return auth_url, local_server, wsgi_app


    def run_local_server_2(self,auth_url, local_server, wsgi_app, results_from_thread,  host='localhost', port=8080,
            authorization_prompt_message=_DEFAULT_AUTH_PROMPT_MESSAGE,
            success_message=_DEFAULT_WEB_SUCCESS_MESSAGE,
            open_browser=True,
            **kwargs):
        print(datetime.datetime.now(), "run2")
        ...
        authorization_response = wsgi_app.last_request_uri.replace(
            'http', 'https')
        print(datetime.datetime.now(), "AUTH RES:", authorization_response)
        print(authorization_prompt_message)
        self.fetch_token(authorization_response=authorization_response)
        results_from_thread[0] = self.credentials
        results_from_thread[1] = authorization_response
        return self.credentials, authorization_response
...
class _RedirectWSGIApp(object):
    ...
    def __call__(self, environ, start_response):
        ...
        """
        # Override the response to redirect to the flask server
        # start_response('301 Moved Permanently', [('Location', 'https://127.0.0.1:5000/login/callback')])
        # start_response('301 Moved Permanently', [('Location', 'http://127.0.0.1:4200')])
        # start_response('301 Moved Permanently', [('Location', 'http://app.wechange.co.uk:5000/login/callback')])
        # start_response('301 Moved Permanently', [('Location', 'https://10.50.1.146:5000/login/callback')])
